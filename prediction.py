...
class Predictor:
    ...
    def predict_anomalies(self, train, test, true_anomalies,  load_scores=False, save_output=True,
        ...
        #ture=np.zeros((test_score.shape))
        if true_anomalies is not None:
            bf_eval = bf_search(test_anomaly_scores, true_anomalies, start=0.01, end=2, step_num=100, verbose=False)
            # bf_eval.update(ndcg(test_score, inter))
            # bf_eval.update(hit_att(test_score,inter))
            # bf_eval.update(ndcg(test_score, inter))
            # bf_eval.update(CR(test_score,inter))
            # bf_eval.update(ER(test_score, inter))
            # bf_eval.update(ER(test_score, inter))
