...
# def hit_att(a,l,ps=[100,150]):
#     res={}
#     for p in ps:
#         a=np.argsort(a).tolist()[::-1]
#         labels=set(np.where(l==1)[0])
#         #if l:
#         size=round(p*len(labels)/100)
#         a_p=set(a[:size])
#         intersect=a_p.intersection(labels)
#         hit=len(intersect)/len(labels)
#         res[f'Hit@{p}%'] = hit
#     return res
#
# def ndcg(a,l,ps=[100,150]):
#     res = {}
#     for p in ps:
