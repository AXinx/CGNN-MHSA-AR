...
def get_data(dataset, max_train_size=None, max_test_size=None,
    ...
    # if str(dataset).startswith("machine"):
    #    try:
    #        f = open(os.path.join(prefix, dataset + "_inter_label.pkl"), "rb")
    #        test_inter = pickle.load(f).reshape((-1, x_dim))[test_start:test_end, :]
    #        f.close()
    #    except (KeyError, FileNotFoundError):
    #        test_inter = None
    ...
    if str(dataset).startswith("machine"):
        #return (train_data, None), (test_data, test_label), test_inter
        return (train_data, None), (test_data, test_label)
    else:
        return (train_data, None), (test_data, test_label), None
